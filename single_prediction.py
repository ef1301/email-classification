...
# cleaning of columns
combined_df = emails.drop(['Unnamed: 0', "label_num"], axis = 1)
# get the subject and body of email
combined_df["subject"] = combined_df["text"].apply(lambda x: get_email_subject(x))
combined_df["body"] = combined_df["text"].apply(lambda x: get_email_body(x))
# ridding of the text column (unless we need it)
combined_df = combined_df.drop(["text"], axis = 1)

# expand default pandas display options to make emails more clearly visible when printed
pd.set_option('display.max_colwidth', 200)

# from here combined_df is our dataframe
#combined_df.head() # you could do print(bodies_df.head()), but Jupyter displays this nicer for pandas DataFrames
jobs1_df=pd.read_csv("input/jobs-1.csv")
combined_df = combined_df.append(jobs1_df, ignore_index=True)
# ----------------------------------------------------------------------------------- PREPROCESSING
...
combined_df["text_clean"] = combined_df["body"].apply(lambda x: utils_preprocess_text(x, flg_tokenize=2, flg_stemm=False, flg_lemm=False, flg_stopwords=True, flg_punct=True))

sns.countplot(x="label",data=combined_df,order=['spam','ham'])
...
# train, test = train_test_split(combined_df, test_size=0.20, stratify=combined_df["label"], random_state=0)

x_train, x_test, y_train, y_test = train_test_split(combined_df["text_clean"], combined_df["label"], test_size=0.2, stratify=combined_df["label"], random_state=0)
...
email_train_df = email_train_df.join(combined_df[["subject","body"]], lsuffix='_caller', rsuffix='_other', how='left')
...
email_test_df = email_test_df.join(combined_df[["subject","body"]], lsuffix='_caller', rsuffix='_other', how='left')
